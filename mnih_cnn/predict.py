        ...
        for y in range(d, args.h_limit, args.map_size):
            for x in range(d, args.w_limit, args.map_size):
                ...
                if len(minibatch) == args.batchsize:
                    queue.append(np.asarray(minibatch, dtype=np.float32))
                    minibatch = []
        queue.append(np.asarray(minibatch, dtype=np.float32))
    queue.append(None)
...
def get_predict(args, ortho, model):
    ...
    patch_queue = []
    create_minibatch(args, ortho, patch_queue)
...
#     canvas_worker.start()
    ...
    while True:
        minibatch = patch_queue.pop(0)
        if minibatch is None:
            break
        minibatch = Variable(
            xp.asarray(minibatch, dtype=xp.float32), volatile=True)
#        preds = model(minibatch, None).data
#        preds = model.extract(minibatch, layers=['fc5'])
        if args.gpu >= 0:
            preds = xp.asnumpy(preds)
        [preds_queue.append(pred) for pred in preds]
    preds_queue.append(None)
    
    tile_patches(args, canvas, preds_queue)
